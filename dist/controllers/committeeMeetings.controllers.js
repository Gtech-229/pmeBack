...
exports.generatePresenceList = exports.signReport = exports.addMeetingReport = exports.updateMeeting = exports.getMeetingDetails = exports.getMeetings = exports.createMeeting = void 0;
...
const generateMeetingList_1 = require("../utils/generateMeetingList");
...
exports.addMeetingReport = (0, express_async_handler_1.default)(async (req, res) => {
    ...
    const report = await prisma_1.prisma.$transaction(async (tx) => {
                ...
                ...(data.projectDecisions && data.projectDecisions.length > 0 && {
                    projectDecisions: {
                        create: data.projectDecisions.map(pd => ({
                            ...
                            tranchesPayload: pd.funding?.tranches ?? client_1.JsonNull
                        }))
                    }
                })
            },
        ...
        const fundingDecisions = data.projectDecisions?.filter(pd => pd.decision === "approved" && pd.funding?.tranches && pd.funding.tranches.length > 0) ?? [];
    ...
    await prisma_1.prisma.$transaction(async (tx) => {
        ...
        for (const op of writeOps) {
            ...
            if (op.newProjectStatus === 'funded') {
                const tranchesPayload = op.decision.tranchesPayload;
 ...
 * **/
exports.generatePresenceList = (0, express_async_handler_1.default)(async (req, res) => {
    ...
    const meeting = await prisma_1.prisma.committeeMeeting.findFirst({
        where: {
            id: meetingId,
            committee: {
                members: {
    ...
    const parsed = committee_schema_1.generatePresenceListSchema.safeParse(req.body);
    if (!parsed.success) {
        res.status(400);
        throw parsed.error;
    }
    ...
    const pdfBuffer = await (0, generateMeetingList_1.generateMeetingReport)(data);

    ...
    if (credits && Array.isArray(credits) && credits.length > 0) {
        await prisma_1.prisma.projectCredit.createMany({
            data: credits.map((c) => {
                ...
                const { monthlyPayment, endDate } = (0, functions_2.computeCreditDetails)({
                    amount,
                    interestRate,
                    durationMonths,
                    startDate,
...
exports.getProjects = (0, express_async_handler_1.default)(async (req, res) => {
    const { status, date, search, campaignId, step, limit, page, sector, maxAge, minAge, hasDisability, type } = req.query;
    ...
    const where = {
        ...
        ...(search ? { OR: [
                ...
                { pme: { name: { contains: search }, promoter: { firstName: { contains: search }, lastName: { contains: search } } } },
            ]
        } : {}),
        ...(campaignId && campaignId !== "all" && { campaignId: campaignId }),
        ...(step && step !== "all" && { currentStepOrder: Number(step) }),
        ...(date && date !== "all" ? (0, functions_1.getDateFilter)(date) : {}),
        ...(sector && sector !== "all" ? { sectorId: search } : {}),
        ...(minAge || maxAge ? (0, functions_1.getAgeFilter)(minAge, maxAge) : {}),
